...
/**
 * <p>The Permission object specifies an inheritable permission tuple on a particular resource.</p><p>The permission tuple relates to a resource, a user or a group, a capability (i.e. something which a user might want to do), a resource type (i.e. the type of object upon which the user might want to perform the action) and a flag indicating whether or not that action is permitted. The permission can relate either to a user or a group (but not both).</p>
 * 
