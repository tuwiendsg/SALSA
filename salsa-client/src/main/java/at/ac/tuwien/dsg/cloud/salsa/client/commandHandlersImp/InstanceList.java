...
//    @Argument(index = 1, usage = "The category of the instance.", metaVar = "category", required = false)
    ...
    public void execute() {
        ...
        try {
            ...
            Logger.getLogger(InstanceList.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void printDataNode(ServiceJsonDataTree node, int level) {
        ...
        } else if (node.getState().equals(SalsaEntityState.UNDEPLOYED.toString())) {
            System.out.format(padding + "%s: %s: %s\n", node.getId(), node.getNodeType(), SalsaEntityState.UNDEPLOYED.getNodeStateString());
        } else {
