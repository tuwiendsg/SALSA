...
public abstract class DeploymentObject_old {
	 ...
	 */
	public abstract void deploy(DeploymentObject_old target);
	 ...
	 */
	public abstract void validate(DeploymentObject_old object);	
	...
	public String getPackageFile(){
		return DeploymentObject_old.class.getProtectionDomain().getCodeSource().getLocation().toString();
	}
	...
	public static DeploymentObject_old importFromXML(String xmlFile){		
		try {
				...
				if (e.getClass().equals(TNodeTypeImplementation.class)){
					...
					for (TDeploymentArtifact art : node.getDeploymentArtifacts().getDeploymentArtifact()) {						
						if (art.getName().equals(KnowledgeArtifactType.salsaClass.getNameString())){
							...
							return (DeploymentObject_old) Class.forName(className).newInstance();
						}
					}
				}
			}			
